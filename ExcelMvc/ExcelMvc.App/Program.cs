namespace ExcelMvc.App
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var content = string.Join(Environment.NewLine, Enumerable.Range(1001, 2000)
                .Select(x => template.Replace("{index}", $"{x}").Replace("{seq}", $"{x-1001}")));
            var names = string.Join(Environment.NewLine, Enumerable.Range(1001, 2000).Select(x => $"L\"Udf{x}\","));
        }
    }
}